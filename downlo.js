...
cmd({
		...
		for ( count ; count < limit ; count++) 
		{
		 // if (count >= limit) break;
		  ...
		  if(isSticker) { let a = await getBuffer(sticUrl); await citel.reply(a, { packname: Config.packname, author: "SONIC-Md"  }, "sticker");} 
...
cmd({
        ...
        async(Void, citel, text) => {
	if (!text) return await citel.send('*Provide Repo Url, Ex:- _.gitclone https://github.com/KALINDU-LK/SONIC-MD_*') 
